...
class MetaDataset(Dataset):
    def __init__(self, dataset, var, t_range=None, dims=None, s=4, subsample_half=False):
        ...
        self.v_sub_dataset = torch.zeros(
            (self.subsample_indices.shape[0], self.v_dataset.shape[1]),
            dtype=torch.float32,
        )
    ...
    def __getitem__(self, idx):
        ...
        meta_dict = {
            "context": {
                "x": self.coords[support_indices],
                "y": sel_volume[support_indices],
            },
...
class PretrainDataset(Dataset):
    def __init__(self, dataset, var, t_range, s=4, split="train"):
        ...
        self.split = split
        self.v_paths = (
            getFilePathsInDir(self.v_dir)[t_range[0] : t_range[1] : 2]
            if split == "train"
            else getFilePathsInDir(self.v_dir)[t_range[0] : t_range[1]]
        )
        self.dims = dims
        self.s = s
        print(self.v_paths)
...
if __name__ == "__main__":
    pretrain_dataset = PretrainDataset(v_dir="/mnt/d/data/vorts/default", dims=[128, 128, 128], t_range=(0, 10), s=4)
