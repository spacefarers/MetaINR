...
heads = [l2l.algorithms.MAML(SIREN(in_features=3, out_features=1, init_features=64, num_res=3).to(config.device), lr=1e-3, first_order=False,allow_unused=True) for _ in range(len(config.test_timesteps)-bundle_size+1)]
backbones = []
replay_buffer = []
...
def train_head(time_step):
    ...
    meta_optimizer = torch.optim.Adam(head.parameters(), lr=1e-4)
